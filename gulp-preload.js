...
export async function preload(done) {
  ...
  var writeStream = fs.createWriteStream("js/controller/preload.js");

  writeStream.write("function preloadImages(expression) {\r\n");
  // [start] switch
  ...
  await deepListDir("images/general").then(function (item) {
    ...
    while (i < len) {
      if (!item[i].includes("DS_Store"))
        writeStream.write("'" + item[i].replace(/\\/g, "/") + "', \r\n");
      i++;
    }
  });
  ...
  while (i < len) {
    ...
    await writeStream.write("case '" + pages[i] + "': \r\n");
