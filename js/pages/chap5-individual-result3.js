var pageId = "#chap5-individual-result3";
var swiper;

function onFlipPrevButtonClicked() {
  _goto("chap5-individual-game3", "prev");
}

function onFlipNextButtonClicked() {
  _goto("closing-page-1", "next");
}
...
function gameHandle() {
  ...
  let answer = $store["#chap5-individual-game3"].indivAns;
  let keys = Object.keys(answer);
  for (var i = 1; i <= keys.length; i++) {
    var value = answer[keys[i - 1]];
    $(`.ques${i}-ans`).html(window[`game3-ques${i}-ans${value}`]);
  }

  swiper = new Swiper("#chap5-individual-result3 .swiper", {
