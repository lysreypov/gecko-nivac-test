var pageId = "#chap5-individual-result2";
var swiper;

function onFlipPrevButtonClicked() {
  _goto("chap5-individual-game2", "prev");
}

function onFlipNextButtonClicked() {
  _goto("chap5-individual-game3", "next");
}
...
function gameHandle() {
  ...
  let answer = $store["#chap5-individual-game2"].indivAns;
  let keys = Object.keys(answer);
  for (var i = 1; i <= keys.length; i++) {
    var value = answer[keys[i - 1]];
    $(`.ques${i}-ans`).html(window[`game2-ques${i}-ans${value}`]);
  }

  swiper = new Swiper("#chap5-individual-result2 .swiper", {
